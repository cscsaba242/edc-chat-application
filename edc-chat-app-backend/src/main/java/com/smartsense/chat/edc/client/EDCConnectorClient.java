...
@FeignClient(name = "edc", url = "https://urban-space-waddle-q77qx5j74v3696-8182.app.github.dev")
public interface EDCConnectorClient {
    ...
    @PostMapping(value = "/management/v3/policydefinitions", consumes = APPLICATION_JSON_VALUE, produces = APPLICATION_JSON_VALUE)
    void createPolicy(URI baseUri,
                      @RequestBody Map<String, Object> request,
                      @RequestHeader("X-Api-Key") String auth);

    @PostMapping(value = "/management/v3/contractdefinitions", consumes = APPLICATION_JSON_VALUE, produces = APPLICATION_JSON_VALUE)
    void createContractDefinition(URI baseUri,
                                  @RequestBody Map<String, Object> request,
                                  @RequestHeader("X-Api-Key") String auth);

    @PostMapping(value = "/management/v3/catalog/request", consumes = APPLICATION_JSON_VALUE, produces = APPLICATION_JSON_VALUE)
    Map<String, Object> queryCatalog(URI baseUri,
                                     @RequestBody Map<String, Object> request,
                                     @RequestHeader("X-Api-Key") String auth);

    @PostMapping(value = "/management/v3/edrs", consumes = APPLICATION_JSON_VALUE, produces = APPLICATION_JSON_VALUE)
    Map<String, Object> initNegotiation(URI baseUri,
                                        @RequestBody Map<String, Object> request,
                                        @RequestHeader("X-Api-Key") String auth);

    @GetMapping(value = "/management/v3/contractnegotiations/{negotiationId}", produces = APPLICATION_JSON_VALUE)
    Map<String, Object> getAgreement(URI baseUri,
                                     @PathVariable("negotiationId") String negotiationId,
                                     @RequestHeader("X-Api-Key") String auth);


    @PostMapping(value = "/management/v3/edrs/request", consumes = APPLICATION_JSON_VALUE, produces = APPLICATION_JSON_VALUE)
    List<Map<String, Object>> initTransferProcess(URI baseUri,
                                                  @RequestBody Map<String, Object> request,
                                                  @RequestHeader("X-Api-Key") String auth);

    @GetMapping(value = "management/v3/edrs/{transferProcessId}/dataaddress", produces = APPLICATION_JSON_VALUE)
